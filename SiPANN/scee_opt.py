...
def bezier_quick(g, l):
    ...
    return {
        ...
        "w": l,
        "f": lambda t: np.sum(
            np.array([(g[j]) * bernstein_quick(n, j, t / l) for j in range(len(g))]),
            axis=0,
        ),
        "df": lambda t: np.sum(
            np.array(
                [
                    ...
                    * (g[j])
                    * (
                        bernstein_quick(n - 1, j - 1, t / l)
                        - bernstein_quick(n - 1, j, t / l)
                    )
        ...
        "d2f": lambda t: np.sum(
            np.array(
                [
                    ...
                    * (g[j] / 2)
                    * (
                        bernstein_quick(n - 2, j - 2, t / l)
                        - 2 * bernstein_quick(n - 2, j - 1, t / l)
                        + bernstein_quick(n - 2, j, t / l)
                    )
                    for j in range(len(g))
                ]
            ),
