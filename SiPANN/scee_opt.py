...
def bezier_quick(g,l):
    ...
    return {'g': g,
            'w': l,
            'f': lambda t: np.sum(np.array([(g[j])*bernstein_quick(n,j,t/l) for j in range(len(g))]),axis=0),
            'df': lambda t: np.sum(np.array([n*(g[j])*(bernstein_quick(n-1,j-1,t/l)-bernstein_quick(n-1,j,t/l)) for j in range(len(g))]),axis=0)/l,
            'd2f': lambda t: np.sum(np.array([n*(n-1)*(g[j]/2)*(bernstein_quick(n-2,j-2,t/l)-2*bernstein_quick(n-2,j-1,t/l)+bernstein_quick(n-2,j,t/l)) for j in range(len(g))]),axis=0)/l**2}
...
def make_coupler(goalK=.4, arrayK=None, waveSweep=np.linspace(1500,1600,4), gapN=16, algo=35, edgeN=8, plot=False, collectData=False, width=500, thickness=220, radius=5000, maxiter=None, verbose=0):
    ...
    scatter1 = axes[0].scatter(np.linspace(0,couplingWidth,gapN),np.zeros(gapN),label='control points')
    line1, = axes[0].plot(np.linspace(0,couplingWidth,500), np.zeros(500),label='gap function')
    line2, = axes[1].plot(waveSweep, np.zeros_like(waveSweep), 'o-',label='cross-port goal = ' + str(goalK))
    axes[1].set_ylim(-5,0)
    ...
    if collectData:
        h5file = open_file('./data/' + ('%.2f'%goalK).split('.')[1] + '/' + "data.h5", mode='w', title = "Simulation Data")
    ...
    def f(g,grad):
        ...
        g_total = np.append(np.append(np.append(np.full(edgeN,gapMax),g[1:]),g[-1:0:-1]),np.full(edgeN,gapMax))

        # get interpolation of waveguide
        gap = bezier_quick(g_total,g[0])
        coupler.update(gap=gap['f'], dgap=gap['df'], zmax=gap['w'])
        curvatureFunc = gap['d2f']
        ...
        if verbose == 2:
            ...
            if localOpt:
                ...
                o = "GLOBAL"
            loop.update(1)
            loop.set_description(f"{o}, MSE: {np.round(mse,4)}, Mean currK: {np.round((np.abs(currK)**2).mean(),4)}")

        return mse

    def constraint(x,grad,radius=5000):
        """
        Constraint function on waveguide curvature to make sure coupler reduces bending loss
        ...
        domain = np.linspace(.002,x[0]-.002,500)
    ...
    gap_total = np.append(np.append(np.append(np.full(edgeN,gapMax),gap_final[1:]),gap_final[-1:0:-1]),np.full(edgeN,gapMax))
    x = np.linspace(0, gap_final[0], 2 * gapN + 2 * edgeN)
    ...
    if plot:
        ...
        axes[0].fill_between(dom,gap['f'](dom)/2,gap['f'](dom)/2+width,color='k')
        axes[0].fill_between(dom,-gap['f'](dom)/2-width,-gap['f'](dom)/2,color='k')
        axes[0].get_legend().remove()
...
def save_coupler(width, thickness, control_pts, length, filename):
    ...
    """
    np.savez(filename, WIDTH=np.array([width]), THICKNESS=np.array([thickness]), LENGTH=np.array([length]), GAP=control_pts)
...
def premade_coupler(split):
    ...
    data = np.load(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'COUPLER', filename))
    g = data['GAP']
    l = data['LENGTH'][0]
    width = data['WIDTH'][0]
    thickness = data['THICKNESS'][0]
