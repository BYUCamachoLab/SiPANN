...
class racetrack_sb_rr():
    '''Racetrack waveguide arc used to connect to a racetrack directional coupler. Ports 
    labeled as::
    ...
    def predict(self, wavelength):
        ...
        TE0_B = np.squeeze(bentWaveguide(wavelength=wavelength*1e-3,width=self.width*1e-3,thickness=self.thickness*1e-3,sw_angle=self.sw_angle,radius=self.radius*1e-3))
        ...
        TE0 = np.squeeze(straightWaveguide(wavelength=wavelength*1e-3,width=self.width*1e-3,thickness=self.thickness*1e-3,sw_angle=self.sw_angle))
